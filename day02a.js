...
while (characterIdx < input.length)
{
	...
	do
	{
		...
		var nextDelimiter = input.indexOf('\t', characterIdx);
		...
		else if (nextNewline >= 0 && nextNewline < nextDelimiter)
		{
			nextDelimiter = nextNewline;
			lastNumber = true;
		}

		var number = parseInt(input.substring(characterIdx, nextDelimiter));

		smallest = Math.min(smallest, number);
		largest = Math.max(largest, number);

		characterIdx = nextDelimiter + 1;
